  ...
  Fs.readdir(`./Commands/${Category}`, (error, Files) => {
    ...
    Files.forEach(File => {
      ...
      if (!Cmd.help.name || !Cmd.help.aliases)
        return console.log(
          `${Cmd.help.name ? Cmd.help.name : "?"} Failed To Load - ❌`
        );
...
Client.on("message", async message => {
  ...
  const Prefix =
    (await Client.Db.fetch(`Prefix_${message.guild.id}`)) || Default_Prefix;
      ...
      if (Now < ExpireTime) {
        const Embed = new Discord.MessageEmbed()
          ...
          .setDescription(
            ...
            })}** Before Using ${Command.help.name.charAt(0).toUpperCase() +
              Command.help.name.slice(1)} Command Again`
          )
          .setTimestamp();
...
Client.login(Token).catch(() =>
  console.log(new Error("Invalid Discord Bot Token Provided!"))
);
